...
class NetworkManager(object):
    def __init__(self, options, path):
        ...
        train_dir = "/workspace/CUB_200_2011/train"
        test_dir = "/workspace/CUB_200_2011/test"
    
        train_set = torchvision.datasets.ImageFolder(train_dir, train_transform_list)
        test_set = torchvision.datasets.ImageFolder(test_dir, test_transforms_list)
    
        self.train_loader = torch.utils.data.DataLoader(train_set, batch_size=64, shuffle=True, num_workers=16, pin_memory=True)
        self.test_loader = torch.utils.data.DataLoader(test_set, batch_size=32, shuffle=False, num_workers=16, pin_memory=True)
    
        
    
    def train(self):
        checkpoint_path = os.path.join(self.path['model_save'], '{net}{choice}-{epoch}-{type}.pth')
        ...
        for epoch in range(self.options['epochs']):
            ...
            if test_acc_epoch>best_acc:
                ...
                best_epoch = epoch+1
                print('*', end='')
                torch.save(model.state_dict(), checkpoint_path.format(net=self.options['net_choice'], choice = self.options['model_choice'], epoch=epoch, type='best'))
